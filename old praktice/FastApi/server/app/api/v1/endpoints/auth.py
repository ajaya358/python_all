...
@limiter.limit("5/minute")
def login_json(request: Request, login_data: LoginRequest, db: Session = Depends(get_login_db)):
    return auth_service.login(db, email=login_data.email, password=login_data.password)
